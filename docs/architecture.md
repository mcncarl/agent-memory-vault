...
`agent_memory_doctor.py` 是统一体检入口，核对 Markdown、SQLite、FTS、INDEX、Zvec 路径与 hash、Git 基线、验证来源、日志隐私、Runtime manifest、模型文件 hash、依赖锁、完全离线语义查询和自动化新鲜度。默认只读；`--repair-derived` 也只重建可再生索引。
